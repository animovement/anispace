...
test_that("map_to_cartesian_polar() drops polar columns and creates x/y", {
  df_in <- make_polar_df()
  df_out <- map_to_cartesian_polar(df_in) |> anicore::as_anipoint()
...
test_that("map_to_cartesian_cylindrical() retains original z", {
  ...
  df_out <- map_to_cartesian_cylindrical(df_in) |> anicore::as_anipoint()
...
test_that("map_to_cartesian_spherical() correctly computes z via spherical_to_z()", {
  ...
  df_out <- map_to_cartesian_spherical(df_in) |> anicore::as_anipoint()
...
test_that("map_to_cartesian() dispatches to the correct helper (polar)", {
  ...
  df_correct <- map_to_cartesian_polar(df_in) |> anicore::as_anipoint()
...
test_that("map_to_cartesian() dispatches to the correct helper (cylindrical)", {
  df_in <- make_cylindrical_df()
  df_out <- map_to_cartesian(df_in)
  df_correct <- map_to_cartesian_cylindrical(df_in) |> anicore::as_anipoint()
...
test_that("map_to_cartesian() dispatches to the correct helper (spherical)", {
  df_in <- make_spherical_df()
  df_out <- map_to_cartesian(df_in)
  df_correct <- map_to_cartesian_spherical(df_in) |> anicore::as_anipoint()

  expect_identical(df_out, df_correct)
})

test_that("map_to_cartesian() aborts with a clear message for unknown systems", {
  # The frame has to be a valid aniframe, or `ensure_is_anipoint()` aborts
  ...
  unknown_df <- suppressWarnings(
    dplyr::tibble(keypoint = "a", time = 1, u = 1, v = 2) |>
      anicore::as_anipoint(variables_where = c("u", "v"))
  )
...
test_that("map_to_cartesian() rejects objects that are not anipoints", {
  expect_error(
